...
// MARK: - Built‑in Echo adapter (proof‑of‑concept)
public struct EchoAdapter: AgentAdapter {
    ...
    public init(cardCapabilities: AgentCard.AgentCapabilities = .init(streaming: true, pushNotifications: false, stateTransitionHistory: false), skills: [AgentCard.AgentSkill] = [AgentCard.AgentSkill(id: "echo", name: "Echo Service", description: "Echoes back user text with a greeting", tags: ["echo"], examples: ["Hello there!"])], defaultInputModes: [String] = ["text/plain"], defaultOutputModes: [String] = ["text/plain"]) {
    ...
    public func handleSend(_ params: MessageSendParams, store: TaskStore) async throws -> A2ATask {
        ...
        // create history w/ user msg
        let message = A2AMessage(role: params.message.role, parts: params.message.parts, messageId: UUID().uuidString, taskId: taskId, contextId: contextId)
        var task = A2ATask(id: taskId,
                           contextId: contextId,
                           status: TaskStatus(state: .submitted, message: message, timestamp: ISO8601DateFormatter().string(from: .init())),
                           history: [params.message])
        
        await store.addTask(task: task)
        _ = await store.updateTask(id: taskId, status: TaskStatus(state: .working, message: message, timestamp: ISO8601DateFormatter().string(from: .init())))
        
        // synthesize reply
        let partText: String = {
            if case .text(let text) = params.message.parts.first {
                return text
            } else {
                return ""
            }
        }()
        let replyText = "Echo Agent: You said '\(partText)'."
        let replyMsg  = A2AMessage(role: "agent", parts: [.text(text: replyText)], messageId: UUID().uuidString, taskId: taskId, contextId: contextId)
        
        _ = await store.updateTask(id: taskId, status: TaskStatus(state: .completed, message: replyMsg, timestamp: ISO8601DateFormatter().string(from: .init())))
        
        task = await store.getTask(id: taskId)! // pull final state
        
        // embed timestamps
        var aStatus = task.status
        aStatus.state = .completed
        aStatus.timestamp = ISO8601DateFormatter().string(from: .init())
        task.status = aStatus
    ...
    public func handleStream(_ params: MessageSendParams, store: TaskStore, eventSink: @escaping (Encodable) -> Void) async throws {
        ...
        let message = A2AMessage(role: params.message.role, parts: params.message.parts, messageId: UUID().uuidString, taskId: taskId, contextId: contextId)
        let baseTask = A2ATask(id: taskId,
                               contextId: contextId,
                               status: TaskStatus(state: .submitted, message: message, timestamp: ISO8601DateFormatter().string(from: .init())),
                               history: [params.message])
        ...
        let submitted = TaskStatusUpdateEvent(taskId: taskId, contextId: contextId, kind: "status-update",
                                              status: TaskStatus(state: .submitted, message: message, timestamp: ISO8601DateFormatter().string(from: .init())), final: false)
        let submittedResponse = SendStreamingMessageSuccessResponse(jsonrpc: "2.0", id: 1, result: submitted)
        eventSink(submittedResponse)
        
        _ = await store.updateTask(id: taskId, status: TaskStatus(state: .working, message: message, timestamp: ISO8601DateFormatter().string(from: .init())))
        let working = TaskStatusUpdateEvent(taskId: taskId, contextId: contextId, kind: "status-update",
                                            status: TaskStatus(state: .working, message: message, timestamp: ISO8601DateFormatter().string(from: .init())), final: false)
        let workingResponse = SendStreamingMessageSuccessResponse(jsonrpc: "2.0", id: 1, result: working)
        eventSink(workingResponse)
        
        // produce incremental artifact
        let partText: String = {
            if case .text(let text) = params.message.parts.first {
                return text
            } else {
        ...
        let full = "Echo Agent: You said '\(partText)'."
        let words = full.components(separatedBy:" ")
        let artID = UUID().uuidString
        var acc   = ""
        for (i,w) in words.enumerated() {
            acc += w + " "
            let art = Artifact(artifactId: artID,
                               parts:[.text(text: acc.trimmingCharacters(in:.whitespaces))],
                               name:"echo",
                               description:nil,
                               metadata: nil,
                               extensions:[])
            let ev  = TaskArtifactUpdateEvent(taskId: taskId, contextId: contextId, kind:"artifact-update",
                                              artifact: art, append:true,
                                              lastChunk: i == words.count-1, metadata:nil)
            let artifactResponse = SendStreamingMessageSuccessResponse(jsonrpc: "2.0", id: 1, result: ev)
            eventSink(artifactResponse)
        }
        
        // mark completed
        let finalMessage = A2AMessage(role:"agent", parts:[.text(text: full)], messageId: UUID().uuidString, taskId: taskId, contextId: contextId)
        _ = await store.updateTask(id: taskId, status: TaskStatus(state: .completed, message: finalMessage, timestamp: ISO8601DateFormatter().string(from: .init())))
        let done = TaskStatusUpdateEvent(taskId: taskId, contextId: contextId, kind:"status-update",
                                         status: TaskStatus(state:.completed, message: finalMessage, timestamp: ISO8601DateFormatter().string(from: .init())), final: true)
        let doneResponse = SendStreamingMessageSuccessResponse(jsonrpc: "2.0", id: 1, result: done)
        eventSink(doneResponse)
    }
}
